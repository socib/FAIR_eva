...
def color_to_rgb(color, alpha=None):
    if color.startswith("#"):
        return (
            *(int(color[i : i + 2], 16) / 255 for i in range(1, 6, 2)),
...
def create_last_page(logos_CSIC, name_pdf, estilos):
    doc_lastpage = SimpleDocTemplate(
        name_pdf,
        pagesize=letter,
        rightMargin=72,
        leftMargin=72,
    ...
    Story_lastpage.append(
        Paragraph("Descargo de responsabilidad:", estilos["JustifyRight12Bold"])
    )
    Story_lastpage.append(
        Paragraph(
...
def bar_FAIR(data):
    ...
    data_barplot = [
        (
            data["findable"]["result"]["points"],
            data["accessible"]["result"]["points"],
            data["interoperable"]["result"]["points"],
            data["reusable"]["result"]["points"],
        )
    ]
    bc = VerticalBarChart()
...
def bar_rda(data_principle):
    ...
    for i, key in enumerate(list(data_principle.keys())[:-1]):
        bc.bars[(0, i)].fillColor = colors.Color(
            color_to_rgb(data_principle[key]["color"])[0],
...
def indicator_table(data):
    ...
    if data["points"] == 100:
        table = [
            ...
            (_l("Indicator Assesment"), Paragraph(_l("%s.indicator" % data["name"]))),
            (
...
def add_group_indicators(Story, data_indicators, name, estilos):
    ...
    Story.append(
        Paragraph(
            str(data_indicators["result"]["points"]) + "%",
    ...
    for k in data_indicators:
        if k != "result":
            print(k)
            Story.append(
                Paragraph(_l(data_indicators[k]["name"]), estilos["centerBold"])
            )
            Story.append(
                Paragraph(
                    str(data_indicators[k]["score"]["earned"]) + "%",
                    def_style(color=data_indicators[k]["color"]),
                )
            )
            Story.append(
                Table(
                    indicator_table(data_indicators[k]),
                    style=[
                        ("GRID", (0, 0), (-1, -1), 1, colors.grey),
                        ("BACKGROUND", (0, 2), (1, 2), data_indicators[k]["color"]),
                    ],
                )
            )
...
def create_pdf(data, name_pdf_report, item_id, logo_FAIR, logos_CSIC, result_points):
    ...
    estilos.add(
        ParagraphStyle(
            name="center", alignment=1, fontSize=10, spaceAfter=10, fontName="Helvetica"
        )
    )
    estilos.add(
        ParagraphStyle(
    ...
    Story.append(
        Paragraph(
            "Your Digital Object is %f %% FAIR" % round(result_points, 2),
